...
import MarginPageDisplay from './MarginPageDisplay.vue'
...
watch(() => props.show, async (val) => {
  ...
  try { margin.value = await api(`/api/broker-margin/${props.broker.id}`) }
...
<template>
  <div v-if="show" class="modal-overlay" @click.self="emit('close')">
    <div class="modal-box">
      <h3>Margin — {{ cap(broker?.friendly_name || broker?.broker_name || '') }}</h3>
      <div v-if="loading" class="state-msg">Loading...</div>
      <div v-else-if="error" class="state-msg error">{{ error }}</div>
      <MarginPageDisplay v-else-if="margin" :data="margin" />
