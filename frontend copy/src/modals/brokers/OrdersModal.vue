...
import OrdersPageDisplay from './OrdersPageDisplay.vue'
...
watch(() => props.show, async (val) => {
  ...
  loading.value = true; error.value = ''; orders.value = []
  try { orders.value = await api(`/api/broker-orders/${props.broker.id}`) }
...
<template>
  <div v-if="show" class="modal-overlay" @click.self="emit('close')">
    <div class="modal-box">
      ...
      <OrdersPageDisplay v-else-if="orders.length" :data="orders" />
