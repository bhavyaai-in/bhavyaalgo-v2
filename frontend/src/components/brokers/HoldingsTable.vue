...
<template>
  <div>
    <div v-if="data?.data?.totalholding" class="summary-row">
      <div v-for="c in summaryCards" :key="c.key" class="summary-card" :class="{negative: Number(data.data.totalholding[c.key])<0, positive: Number(data.data.totalholding[c.key])>0}">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="h in data.data.holdings" :key="h.symboltoken">
            ...
            <td :class="{negative: Number(pnl(h)) < 0, positive: Number(pnl(h)) > 0}">{{ pnl(h) }}</td>
            <td :class="{negative: Number(h.pnlpercentage) < 0, positive: Number(h.pnlpercentage) > 0}">{{ h.pnlpercentage }}%</td>
          </tr>
        </tbody>
      </table>
    </div>
...
.summary-card.positive .summary-value { color:#16A34A; }
